...
<div class="colorbox">
    ...
    <!-- Php pour afficher les différents manga du site -->
    <?php if($manga->rowCount() > 0) { ?>
        <?php while($a = $manga->fetch()) { ?>
            <div class="active">
            <div class="ajouterM">
            <center><h1 class="ac"><?= $a['titre'] ?></h1></center>
            <br>
            <center><a href="manga.php?manga=<?php echo $a['id']?>"><img src=<?php echo $a['image'] ?> width=60% height=40%></a></center>
            <br>
            </div>
            </div>
        <?php } ?> 
